...
public class UIBinder {
    ...
    public void bind() {
        ...
        if (container == null || model == null) {
            throw new IllegalStateException("Missing: "
                    + (container == null ? "container" : "model"));
        }
        ...
        for (Map.Entry<Integer, PropertyLookup> entry : bindingMap.entrySet()) {
            ...
            if (view instanceof TextView) {
                ...
                if (value instanceof Number) {
                    strVal = defaultNumberFormat.format(value);
                } else {
    ...
    private void setValue(Object model, String property, Object value) {
        ...
        if (idx > 0) {
            setValue(getValue(model, property.substring(0, idx)),
                    property.substring(idx + 1), value);
            return;
        }
            ...
            try {
                ...
                if (String.class == propClass) {
                    ...
                    if (!(value instanceof Integer)) {
                        value = value != null ? Integer.parseInt(value
                                .toString()) : null;
                    }
                } else if (Float.class == propClass) {
                    if (!(value instanceof Float)) {
                        value = value != null ? Float.parseFloat(value
                                .toString()) : null;
                    }
                } else if (Boolean.class == propClass) {
                    if (!(value instanceof Boolean)) {
                        value = value != null ? Boolean.parseBoolean(value
                                .toString()) : null;
                    }
                }
    ...
    private Object getValue(Object model, String property) {
        ...
        int idx = property.indexOf(".");
        if (idx > 0) {
            return getValue(getValue(model, property.substring(0, idx)),
        ...
        try {
            for (Method method : model.getClass().getMethods()) {
                if (method.getName().toUpperCase()
                        .equals(("get" + property).toUpperCase())) {
    ...
    private Method getMutator(Object model, String property) {
        ...
        try {
            for (Method method : model.getClass().getMethods()) {
                if (method.getName().toUpperCase()
                        .equals(("set" + property).toUpperCase())) {
                    return method;
                }
            }
        } catch (SecurityException e) {
